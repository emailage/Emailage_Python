...
if use_urllib_quote:
    def _url_encode_dict(qs_dict):
        return '&'.join(map(lambda pair: '='.join([urllib.quote(text_type(pair[0]), ''),
                                                   urllib.quote(text_type(pair[1]), '')]),
    ...
    # Python 3.x - 3.4
    def _url_encode_dict(qs_dict):
        return '&'.join(map(lambda pair: '='.join([urllib.parse.quote(text_type(pair[0]), ''),
                                                   urllib.parse.quote(text_type(pair[1]), '')]),
                            sorted(qs_dict.items())))
